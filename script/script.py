...
import argparse
import logging
from command_executor import DefaultCommandExecutor
...
def main():
    ...
    executor = DefaultCommandExecutor(logger=logger)

    current_dir = Path(".").resolve()
    logger.debug(f"Checking directory: {current_dir}")

    # Updated function calls with both runner and logger
    if check_if_current_is_git_repo(executor, logger):
        logger.info(f"Directory '{current_dir}' IS a Git repository.")
    else:
        logger.error(f"Directory '{current_dir}' is NOT a Git repository.")
        sys.exit(1)

    actual_branch = get_actual_git_branch(executor, logger)
    logger.info(f"Current directory: {current_dir}")
    logger.info(f"Current branch: {actual_branch}")

    parent_branch = get_direct_branch_parent(executor, logger)
    ...
    target_branch_exists = check_if_branch_exists(executor, logger, target_branch)
    auxiliary_branch_exists = check_if_branch_exists(executor, logger, auxiliary_branch)
    ...
    if not target_branch_exists or not auxiliary_branch_exists:
        ...
        if not auxiliary_branch_exists:
            logger.error(f"Auxiliary branch '{auxiliary_branch}' does not exist.")
        sys.exit(1)
